...
import { calculateFare, VEHICLE_RATES, SURCHARGES_CONFIG } from "./pricing.js";
...
// FARE ESTIMATION
// ============================================
export async function handleEstimateFare(payload) {
  const { origin, destination } = payload || {};

  if (!origin || !destination) {
    return {
      status: 400,
      body: { error: "Both pickup (origin) and destination are required for fare estimation." }
    };
  }
  ...
  try {
    ...
    const fares = {};
    const rates = {};
    for (const key of Object.keys(VEHICLE_RATES)) {
      fares[key] = calculateFare(key, distanceKm);
      rates[key] = {
        baseFare: VEHICLE_RATES[key].baseFare,
        perKmRate: VEHICLE_RATES[key].perKmRate
      };
    }
...
export async function handleCreateBooking(payload, baseUrl) {
  ...
  let calculatedFare = fare || "Fixed Quote on Dispatch";
  if ((vehicle === "4-Seater" || vehicle === "6-Seater") && distanceKm) {
    const calc = calculateFare(vehicle, Number(distanceKm));
    if (calc !== null) {
      calculatedFare = `Estimated SGD ${calc.toFixed(2)}`;
    }
  }
