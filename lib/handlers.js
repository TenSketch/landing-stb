...
import { estimateFaresForAllVehicles, calculateFareDetailed } from "./pricing.js";

function generateSecureVoucher() {
...
// ============================================
// FARE ESTIMATION (Database & Google Routes Driven)
// ============================================
export async function handleEstimateFare(payload, referer) {
  const { origin, destination, bookingType = "One Way", dateTime, hours, days } = payload || {};
  const normMode = bookingType.toLowerCase().includes("hour") ? "HOURLY" : 
                   bookingType.toLowerCase().includes("day") || bookingType.toLowerCase().includes("tour") ? "DAILY" : "ONE_WAY";

  let distanceKm = 0;
  let durationSeconds = 0;
  let originPlaceId = origin?.placeId || null;
  let destPlaceId = destination?.placeId || null;

  if (normMode === "ONE_WAY") {
    if (!origin || !destination) {
      return {
        status: 400,
        body: { error: "Both pickup (origin) and destination are required for one-way fare estimation." }
      };
    }

    const originLoc = getWaypointLocation(origin);
    const destLoc = getWaypointLocation(destination);

    if (!originLoc || !destLoc) {
      return {
        status: 400,
        body: { error: "Invalid origin or destination location format. Must contain placeId or latitude/longitude." }
      };
    }
    ...
    if (!apiKey) {
      return {
        status: 500,
        body: { error: "Google Maps API Key is not configured on the server." }
      };
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error?.message || `Routes API returned status ${res.status}`);
      }
      ...
      if (distanceMeters === undefined) {
        throw new Error("Distance details missing in route response.");
      }

      const durationStr = data.routes[0].duration || "";
      durationSeconds = parseInt(durationStr.replace("s", ""), 10) || 0;
      distanceKm = distanceMeters / 1000;
    } catch (err) {
      console.error("[FARE ESTIMATION] Error calling Google Routes API:", err.message);
      return {
        status: 500,
        body: {
          success: false,
          error: err.message || "Failed to calculate routes distance."
        }
      };
    }
  }
  ...
  try {
    const estimationResult = await estimateFaresForAllVehicles({
      bookingType,
      distanceKm,
      durationSeconds,
      originPlaceId,
    ...
    return {
      status: 200,
      body: estimationResult
    };
  } catch (err) {
    console.error("[FARE ESTIMATION] Pricing calculation failed:", err.message);
    return {
      status: 500,
      body: {
        success: false,
        error: err.message || "Pricing configuration error."
      }
    };
  }
}

function getWaypointLocation(item) {
  if (!item) return null;
  if (item.placeId) {
...
// ============================================
// CREATE BOOKING (Authoritative Server-Side Fare Authority)
// ============================================
export async function handleCreateBooking(payload, baseUrl) {
  const {
    ...
    currency, paymentMethod, pax,
    bookingType, notes,
    pickupPlaceId, pickupCoords, destPlaceId, destCoords,
    distanceKm, hours, days,
  } = payload || {};

  if (!passengerName || !passengerEmail || !passengerPhone) {
    return { status: 400, body: { error: "Passenger name, email, and WhatsApp number are required." } };
  }

  // Authoritative server-side fare calculation (ignore client-supplied amounts)
  const vehicleToUse = vehicle || "4-Seater";
  let calculatedFare = "Pending Verification on Dispatch";
  let authoritativeAmount = null;

  try {
    const fareCalc = await calculateFareDetailed({
      vehicleName: vehicleToUse,
      bookingType: bookingType || "One Way",
      distanceKm: distanceKm ? Number(distanceKm) : 0,
      originPlaceId: pickupPlaceId || null,
      destPlaceId: destPlaceId || null,
      hours: hours ? Number(hours) : 1,
      days: days ? Number(days) : 1,
      dateTime
    });
    if (fareCalc && fareCalc.totalFare) {
      authoritativeAmount = fareCalc.totalFare;
      calculatedFare = `Estimated SGD ${fareCalc.totalFare.toFixed(2)}`;
    }
  } catch (err) {
    console.warn("[BOOKING] Authoritative fare calculation failed, falling back to manual dispatch verification:", err.message);
  }

  const generatedVoucher = generateSecureVoucher();

  const booking = {
    id: generatedVoucher,
    voucherCode: generatedVoucher,
    passengerName, passengerEmail, passengerPhone,
    vehicle: vehicle || "Standard Private Transport",
    pickup, destination, dateTime, flightNo,
    fare: calculatedFare,
    currency: currency || "SGD",
    paymentMethod: paymentMethod || "Pay After Service",
    pax: pax || "1-3 Passengers",
    bookingType: bookingType || "One Way",
    notes: notes || null,
  ...
  const adminWa = (process.env.NEXT_PUBLIC_ADMIN_WHATSAPP_NUMBER || "+6590629107").replace(/[^0-9]/g, "");
  let waText = `*STB Singapore — Transport Booking Inquiry*\n\n`;
  waText += `🎟 *Inquiry Ref:* ${booking.voucherCode}\n`;
  waText += `👤 *Passenger:* ${passengerName}\n`;
  waText += `📱 *WhatsApp:* ${passengerPhone}\n`;
  waText += `✉️ *Email:* ${passengerEmail}\n`;
  waText += `🚘 *Vehicle Choice:* ${booking.vehicle}\n`;
  waText += `🚘 *Booking Type:* ${booking.bookingType}\n`;
  waText += `📍 *Pickup:* ${pickup}\n`;
  if (destination) waText += `🏁 *Destination:* ${destination}\n`;
  waText += `📅 *Date & Time:* ${dateTime || "Flexible"}\n`;
  waText += `👥 *Passengers:* ${pax || "1-3 Passengers"}\n`;
  if (flightNo) waText += `✈️ *Flight:* ${flightNo}\n`;
  if (notes) waText += `📝 *Notes:* ${notes}\n`;
  waText += `💰 *Fare:* ${booking.fare}\n`;
  waText += `💰 *Payment:* No prepayment required (Pay after trip)\n\n`;
  waText += `Please confirm vehicle availability for this inquiry. Thank you!`;
  const whatsappUrl = `https://wa.me/${adminWa}?text=${encodeURIComponent(waText)}`;
  ...
  if (transporter) {
    ...
    try {
      const info = await transporter.sendMail({
        ...
        subject: `STB Transport Inquiry Received — ${booking.voucherCode}`,
      ...
      console.error("[EMAIL] Guest inquiry failed:", err.message);
    }
    ...
    try {
      ...
      const info = await transporter.sendMail({
        ...
        subject: `New STB Transport Inquiry — ${booking.voucherCode}`,
  ...
  return { status: 200, body: { success: true, message: "Transport inquiry received.", booking, emailsSent: emailResult, whatsappUrl } };
}
