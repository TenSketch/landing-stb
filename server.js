...
import { guestEmail, adminEmail, reminderEmail, guestText, adminText, reminderText } from "./emails/templates.js";
...
// ============================================
// Persistence — /app/data/bookings.json
// ============================================
...
function updateBooking(voucherCode, patch) {
...
function makeBrand(req) {
  return {
    ...
    site: process.env.SITE_URL || baseUrl(req),
    assignUrl: req ? `${baseUrl(req)}/assign` : "",
  };
}
...
app.post("/api/bookings", async (req, res) => {
  ...
  if (transporter) {
    const brand = makeBrand(req);
    const adminBrand = { ...brand, assignUrl: `${baseUrl(req)}/assign/${encodeURIComponent(booking.voucherCode)}` };
    ...
    try {
      ...
      const info = await transporter.sendMail({
        ...
        html: adminEmail(booking, adminBrand),
  ...
  res.json({ success: true, message: "Booking confirmed.", booking, emailsSent: emailResult, whatsappUrl });
});

// ============================================
// Driver assignment — GET form + POST save
// ============================================
function assignPageHtml({ booking, message = "", ok = false } = {}) {
  const brand = {
    name: process.env.BRAND_NAME || "STB Singapore",
    logoUrl: process.env.LOGO_URL || "",
  };
  ...
  <div class="container">
    ...
    <div class="header">
      ...
      <p class="sub">Fill in chauffeur details — the guest gets a reminder email 12 hours before pickup.</p>
    </div>
    ...
    <form method="POST" action="/assign/${encodeURIComponent(booking.voucherCode)}">
      ...
      <button type="submit">${ok ? "Update Driver Details" : "Save & Schedule Reminder"}</button>
      <div class="note">You can edit these details anytime by re-opening this link.<br>The 12-hour reminder to <strong>${val("passengerEmail")}</strong> uses the latest data.</div>
    </form>
  </div></body></html>`;
}
...
app.get("/assign", (_req, res) => {
  res.status(200).send(`<!doctype html><html><head><meta charset="utf-8"><title>STB Assign</title>
    ...
    <body><h1>STB Dispatch</h1><p>Open your booking alert email and click the <strong>Assign Driver</strong> button — the link includes the voucher code.</p></body></html>`);
});

app.get("/assign/:voucherCode", (req, res) => {
  const list = loadBookings();
  const booking = list.find((b) => b.voucherCode === req.params.voucherCode);
  res.status(booking ? 200 : 404).send(assignPageHtml({ booking, ok: Boolean(booking?.driverName) }));
});

app.post("/assign/:voucherCode", (req, res) => {
  const { driverName, driverPhone, driverPlate, driverPhotoUrl } = req.body || {};
  const updated = updateBooking(req.params.voucherCode, {
  ...
  const msg = updated ? "✓ Saved. Guest reminder will fire 12h before pickup with these details." : "";
  res.send(assignPageHtml({ booking: updated, message: msg, ok: true }));
});
...
async function tickReminders() {
  ...
  for (const b of list) {
    ...
    // Fire when we're within 12 hours of pickup, and pickup hasn't already passed
    if (untilPickup <= WINDOW_MS && untilPickup > 0) {
      try {
        ...
        const info = await transporter.sendMail({
          ...
          html: reminderEmail(b, brand),
          text: reminderText(b),
        });
        updateBooking(b.voucherCode, { reminderSentAt: new Date().toISOString(), reminderMessageId: info.messageId });
        console.log("[REMINDER] Sent for", b.voucherCode, "→", b.passengerEmail);
      } catch (err) {
...
setTimeout(tickReminders, 8 * 1000);
